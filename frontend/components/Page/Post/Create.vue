...
import type { PostStoreResponse } from "@/types/api/post";

const emit = defineEmits<{
  submit: [];
}>();

async function submit(body: any, node: FormKitNode) {
  const { data, error } = await useFetchAPI<ApiResponse<PostStoreResponse>>(
    "/posts",
    {
      method: "post",
      body: body,
